RSpec.shared_examples 'route' do |verb, path|
  ...
  def body(raw_body)
    return ['get', 'delete'].include?(verb) ? raw_body : raw_body.to_json
  end
  ...
  describe 'No application key error' do
    before do
      public_send(verb.to_sym, path, {session_id: valid_session.id.to_s})
    end
  ...
  describe 'No session identifier error' do
    before do
      public_send(verb.to_sym, path, {app_key: application.key})
    end
  ...
  describe 'Unknown application error' do
    before do
      public_send(verb.to_sym, path, {app_key: 'any unknown key', session_id: valid_session.id.to_s})
    end
  ...
  describe 'Unknown session error' do
    before do
      public_send(verb.to_sym, path, {app_key: application.key, session_id: 'any unknown id'})
    end
  ...
  describe 'Invalid session error' do
    before do
      public_send(verb.to_sym, path, {app_key: application.key, session_id: invalid_session.id.to_s})
    end
  ...
  describe 'Unauthorized error' do
    ...
    before do
      public_send(verb.to_sym, path, {app_key: application.key, session_id: valid_session.id.to_s})
    end
